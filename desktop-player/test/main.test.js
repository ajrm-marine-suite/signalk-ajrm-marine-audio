...
assert.match(rendererSource, /document\.querySelectorAll\("\[data-tab\]"\)/);
assert.match(rendererSource, /function selectTab\(tabName\)/);
assert.match(rendererSource, /panel\.hidden = !active/);
...
assert.match(rendererSource, /settings\.keepAliveVolume = clampPercent\(settings\.keepAliveVolume, 50\)/);
assert.match(rendererSource, /els\.keepAliveAudio\.volume = clampPercent\(settings\.keepAliveVolume, 50\) \/ 100/);
assert.match(rendererSource, /function clampPercent\(value, fallback\)/);
assert.match(rendererSource, /configureKeepAliveTimer\(\)/);
assert.match(rendererSource, /addEventListener\("input", applyKeepAliveIntervalInput\)/);
assert.match(rendererSource, /function scheduleNextKeepAlivePulse\(\)/);
assert.match(rendererSource, /seconds \* 1000/);
assert.match(rendererSource, /keep-alive-armed/);
